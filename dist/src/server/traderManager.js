...
exports.getTraderStore = getTraderStore;
exports.recordTraderTransaction = recordTraderTransaction;
exports.subscribeToTrader = subscribeToTrader;
exports.getTraderStats = getTraderStats;
exports.cleanupInactiveTraders = cleanupInactiveTraders;
const ws_1 = require("ws");
const tokens_1 = require("../types/tokens");
const traderutils_1 = require("../utils/traderutils");
const tokenManager_1 = require("./tokenManager");
function updateTraderProfitTier(ws, traderAddress, profit, solanaPrice) {
    if (!global.pumpFunState || !global.pumpFunState.traderStore) {
        console.log("Global state or traderStore not initialized, skipping profit tier update");
        return;
    }
    ...
    if (!traderData) {
        ...
        console.log(`Created new trader data for ${traderAddress} with initial profitTier: null`);
    }
    const newTotalProfit = traderData.totalProfit + profit;
    const oldTier = traderData.profitTier;
    const newTier = (0, traderutils_1.getTraderProfitTier)(newTotalProfit, solanaPrice);
    ...
    if (oldTier !== newTier) {
        ...
        if (newTier) {
        ...
        else if (global.pumpFunState.subscribedTraders.has(traderAddress) &&
            ...
            try {
                ...
                console.log(`Unsubscribed from trader ${traderAddress} (tier became null)`);
            }
            catch (error) {
                console.error(`Error unsubscribing from trader ${traderAddress}:`, error);
            }
        }
        // Update tokenTraderTiers for all tokens the trader has traded
        const tradedMints = new Set(traderData.transactions.map((tx) => tx.mint));
        tradedMints.forEach((mint) => {
            if (newTier !== null) {
                (0, tokenManager_1.updateTokenTraderTiers)(mint, traderAddress, newTier);
                (0, tokenManager_1.updateTierStats)(mint); // Ensure tiertransactions and stats are updated
            }
        });
    }
}
function getTraderStore() {
...
function recordTraderTransaction(ws, traderAddress, mint, txType, amount, tokenAmount, profit, signature) {
    if (!global.pumpFunState || !global.pumpFunState.traderStore)
        return;
    ...
    const isexistingTransaction = traderData.transactions.find((tx) => tx.signature === signature &&
        tx.txType === txType &&
        tx.mint === mint);
    ...
    if (!isexistingTransaction) {
        ...
        if (txType === "buy" && traderData.profitTier && typeof tokenAmount === "number") {
            (0, tokenManager_1.updateTokenTraderTiers)(mint, traderAddress, traderData.profitTier);
        }
...
function subscribeToTrader(ws, traderAddress) {
    if (!ws || ws.readyState !== ws_1.WebSocket.OPEN || !global.pumpFunState) {
        console.error("Cannot subscribe to trader: WebSocket not open or global state not initialized");
        return;
    }
    try {
...
function cleanupInactiveTraders(ws) {
    if (!global.pumpFunState || !ws || ws.readyState !== ws_1.WebSocket.OPEN) {
        console.log("Global state or WebSocket not initialized, skipping trader cleanup");
        return;
    }
    const now = new Date();
    ...
    for (const [address, trader] of global.pumpFunState.traderStore) {
        const daysInactive = (now.getTime() - trader.lastActiveTimestamp.getTime()) /
            (1000 * 60 * 60);
        if (daysInactive > 24) {
    ...
    if (inactiveTraders.length > 0) {
        ...
        for (let i = 0; i < inactiveTraders.length; i += CHUNK_SIZE) {
            ...
            if (chunk.length === 0)
                continue;
            try {
                ws.send(JSON.stringify({ method: "unsubscribeAccountTrade", keys: chunk }));
                console.log(`Unsubscribed from ${chunk.length} inactive traders (batch ${Math.floor(i / CHUNK_SIZE) + 1})`);
            }
            catch (error) {
                console.error(`Error unsubscribing from traders:`, error);
            }
        }
        console.log(`Cleaned up ${inactiveTraders.length} inactive traders. Trader store size: ${initialTraderCount} -> ${global.pumpFunState.traderStore.size}`);
    }
    else {
        console.log(`No inactive traders to clean up. Trader store size: ${initialTraderCount}`);
    }
}
