...
function initWebSocketServer(server) {
    ...
    wss.on("connection", (ws) => {
        ...
        try {
            ...
            const { tokens, solanaPrice, lastPriceUpdate } = (0, tokenManager_1.getTokenStore)();
            console.log("Client connected, current tokenStore length:", tokens.length);
            if (tokens.length > 0) {
                console.log("Sending initial data to client, first token:", tokens[0].mint);
                try {
            ...
            ws.on("message", (message) => {
                try {
                    ...
                    const data = (0, utils_1.safeJsonParse)(messageStr);
                    if (!data)
                        return;
