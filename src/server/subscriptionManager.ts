...
async function sendBatchSubscriptions(
  ws: WebSocket,
  addresses: string[],
  method: "subscribeTokenTrade" | "subscribeAccountTrade",
...
export async function subscribeToAll(ws: WebSocket): Promise<void> {
  if (!ws || ws.readyState !== WebSocket.OPEN) return;

  initGlobalState();
  if (!global.pumpFunState) throw new Error("Global state not initialized");
  ...
  const newTokens = tokenStore
    .filter((token) => token?.mint && !isTokenOlderThan24Hours(token) && !subscribedTokens.has(token.mint))
  ...
  if (newTokens.length === 0 && newTraders.length === 0) return;
