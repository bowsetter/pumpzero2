...
import { initGlobalState,saveGlobalState } from "./globalState";
...
export function getPumpFunStateSize(): number {
  if (!global.pumpFunState) {
    return 0;
  }

  // Serialize the object to JSON
  const jsonString = JSON.stringify(global.pumpFunState, (key, value) => {
    // Handle non-serializable types like Map, Set, and EventEmitter
    if (value instanceof Map) {
      return Array.from(value.entries());
    } else if (value instanceof Set) {
      return Array.from(value);
    } else if (value instanceof EventEmitter) {
      return null; // Exclude EventEmitter or handle minimally
